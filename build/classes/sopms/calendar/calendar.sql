...
-- 삽입 트리거
CREATE OR REPLACE TRIGGER insert_cal
AFTER INSERT ON wbs
FOR EACH ROW
BEGIN
	INSERT INTO calendar VALUES (cal_seq.nextval, :NEW.workcode, :NEW.title, :NEW.content, to_char(:NEW.start_date,'YYYY-MM-DD"T"00:00:00.000"Z"'), to_char(:NEW.end_date,'YYYY-MM-DD"T"00:00:00.000"Z"'), 1, '#6633FF', '#6633FF', '#FFFFFF', :NEW.pm, :NEW.manager, :NEW.parent, NULL, :NEW.status);
END;
...
SELECT a.*, b.name
	...
	SET status = 95
	WHERE id = 26;
ALTER TABLE calendar ADD status varchar2(100);
UPDATE calendar
	SET status = '승인요청'
	WHERE id = '13';
UPDATE calendar
